...
function build_mail_tfa(name, code) {
  return `<!DOCTYPE html>
  <html>
  <head>
	<meta charset="utf-8" />
...
function build_mail_invitation(name, code) {
  return `<!DOCTYPE html>
  <html>
  <head>
	<meta charset="utf-8" />
		  ...
		  <table class="container" width="600" cellpadding="0" cellspacing="0">
			...
			<tr>
			  <td>
				...
				<p>
				  Ingresa este código para continuar con la partida. 
				  Si tú no solicitaste este código, puedes ignorar este mensaje.
				</p>
				<p>Gracias,<br>El equipo de Soporte</p>
			  </td>
...
export async function send_mail_invitation(mail, name, code) {
  /* Set the mail config */
  const options = {
    from: '"Transcendence" <no-reply@transcendence.com>',
    to: mail,
    subject: "Game on Transcendence - Invitation Code",
    html: build_mail_invitation(name, code),
  };
