...
import { send_mail_tfa } from "../utils/mail_config.js";
...
async function manage_user(users_api, name, email, picture) {
  /* Get the list */
  const users_response = await fetch(`${users_api}/?email=${email}`);
...
export default async function google_callback(request, reply) {
  ...
  try {
    /* Get the tokens and send a request to get the info */
    const { name, email, picture } = await google_authentication(
      request,
      reply
    );

    /* Manage the user creation or search */
    const { user_id, user_name, user_email, language, tfa } = await manage_user(
      users_api,
      name,
      email,
      picture
    ...
    db.prepare(
      "INSERT INTO tfa_codes(hash, user_id, language, code) VALUES(?, ?, ?, ?)"
    ).run(hash, user_id, language, code);

    /* Send the mail */
    await send_mail_tfa(user_email, user_name, code);

    /* Redirect with tfa true and the temporal code */
    return reply.redirect(
      `${process.env.FRONTEND_BASEURL_EXTERNAL}/login/tfa?hash=${hash}`
    );
  } catch (e) {
