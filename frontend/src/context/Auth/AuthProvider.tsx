...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const refreshUser = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_AUTH_API_BASEURL_EXTERNAL}/me`,
        {
          credentials: "include",
        }
      );
  ...
  const login = useCallback(() => {
    window.location.assign(
      `${import.meta.env.VITE_AUTH_API_BASEURL_EXTERNAL}/login`
    );
  }, []);
  ...
  const logout = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_AUTH_API_BASEURL_EXTERNAL}/logout`,
        {
          credentials: "include",
        }
      );
  ...
  const deleteUser = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_USER_API_BASEURL_EXTERNAL}/${user.id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
