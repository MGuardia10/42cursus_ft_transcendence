  ...
  return (
    <div className="flex flex-col gap-2 md:gap-6 h-full p-5 sm:p-6 md:p-8 w-full">
      ...
      <div className="flex items-center text-sm md:text-base">
        <button
          className={`px-2 md:px-4 hover:cursor-pointer ${
            view === "friends"
              ? "text-text-secondary"
              : "hover:text-text-tertiary transition-all duration-300"
          }`}
      ...
      {view === "friends" && (
        <div className="grid grid-cols-1 gap-3 overflow-y-scroll scrollbar-thin">
          {friends.map((friend) => (
            <div
              key={friend.id}
              className="group flex justify-between px-4 py-3 rounded-sm bg-background-hover border-2 border-background-secondary"
            >
              <div className="flex items-center">
                <img
                  src={friend.avatar}
                  alt={friend.alias}
                  className="w-12 h-12 rounded-full border-2 border-text-tertiary mr-4 object-cover"
                />
                <div>
      ...
      {view === "requests" && (
        <div className="grid grid-cols-1 gap-3 overflow-y-scroll scrollbar-thin">
          {pending
            .filter((friend): friend is Friend => friend.from === friend.id)
            .map((request) => (
              <div
                key={request.id}
                className="flex items-center px-4 py-3 rounded-sm bg-background-hover border-2 border-background-secondary"
              >
                <img
                  src={request.avatar}
                  alt={request.alias}
