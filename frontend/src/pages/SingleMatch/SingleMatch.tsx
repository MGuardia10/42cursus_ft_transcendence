...
const SingleMatch: React.FC = () => {
  ...
  const gameLoop = useCallback(() => {
    setGameState(prev => {
      ...
      newState.enemyPaddle.y += newState.enemyPaddle.direction * ENEMY_SPEED;
      if (newState.enemyPaddle.y <= 0 || newState.enemyPaddle.y >= gameHeight - PADDLE_HEIGHT) {
        newState.enemyPaddle.direction *= -1;
      }
      ...
      <div className="w-full max-w-6xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-text-secondary">
            {t?.singleMatchDescription || 'Play Pong against a simple AI opponent'}
          </p>
        </div>
        ...
        <div className="flex justify-center items-center mb-6 bg-background-secondary rounded-lg p-4">
          <div className="text-center mx-8">
            <div className="text-text-secondary text-sm mb-1">{t?.player || 'Player'}</div>
          ...
          <div className="text-center mx-8">
            <div className="text-text-secondary text-sm mb-1">{t?.computer || 'Computer'}</div>
        ...
        <div className="flex justify-center mb-6">
          <div
            ref={gameRef}
            className="relative bg-background-secondary border-2 border-border-primary rounded-lg overflow-hidden"
            style={{ width: '100%', maxWidth: '600px', height: 'auto', aspectRatio: '4/3' }}
          >
            <div className="absolute bg-border-primary opacity-50" style={{ left: '50%', top: 0, width: '2px', height: '100%', transform: 'translateX(-50%)' }} />
            <div className="absolute bg-text-tertiary rounded-sm" style={{ left: 0, top: `${(gameState.playerPaddle.y / gameState.gameHeight) * 100}%`, width: `${(PADDLE_WIDTH / gameState.gameWidth) * 100}%`, height: `${(PADDLE_HEIGHT / gameState.gameHeight) * 100}%` }} />
            <div className="absolute bg-text-secondary rounded-sm" style={{ right: 0, top: `${(gameState.enemyPaddle.y / gameState.gameHeight) * 100}%`, width: `${(PADDLE_WIDTH / gameState.gameWidth) * 100}%`, height: `${(PADDLE_HEIGHT / gameState.gameHeight) * 100}%` }} />
            <div className="absolute bg-text-primary rounded-full" style={{ left: `${(gameState.ball.x / gameState.gameWidth) * 100}%`, top: `${(gameState.ball.y / gameState.gameHeight) * 100}%`, width: `${(BALL_SIZE / gameState.gameWidth) * 100}%`, height: `${(BALL_SIZE / gameState.gameHeight) * 100}%` }} />
            {!gameState.gameStarted && (
              <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                <div className="text-2xl text-text-primary">
                  {gameState.score.player === 0 && gameState.score.enemy === 0 ? (t?.pressToStart || 'Press Start to Play') : (t?.gamePaused || 'Game Paused')}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          <div className="text-center text-text-secondary text-sm max-w-md">
            <p className="mb-2 ">{t?.controls || 'Controls:'} ↑↓ {t?.or || 'or'} W/S {t?.toMove || 'to move your paddle'}</p>
            <p>{t?.firstToScore || 'First to score wins!'}</p>
          </div>
        </div>
      </div>
    </div>
